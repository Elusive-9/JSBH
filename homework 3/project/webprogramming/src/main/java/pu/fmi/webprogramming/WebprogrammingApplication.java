...
public class WebprogrammingApplication {
	...
	public static void main(String[] args) {
		...
		ApplicationContext springContainer = SpringApplication.run(WebprogrammingApplication.class, args);
		...
		DeliveryServiceInterface deliveryServiceInterface = springContainer.getBean(DeliveryServiceInterface.class);

		LOG.info("Deliver Service: {}", deliveryServiceInterface);

		DeliveryServiceInterface deliveryServiceInterface2 = springContainer.getBean(DeliveryServiceInterface.class);

		LOG.info("Deliver Service 2: {}", deliveryServiceInterface2);

		Customer customer = new Customer(1L, "Petar", "Ivanov", "petar.ivanov", "0123456789", "Plovdiv");
	...
	// 2. @Bean трябва да се намира над метод - в клас, който е анотиран с
	// @Configuration
