...
public class DeliveryApi {
	...
	public Delivery updateDeliveryCourier(@PathVariable Long id, @RequestParam Long courierId) {
		return deliveryServiceInterface.assignCourier(id, courierId);
	}

	@PutMapping("/{id}") // PUT /api/deliveries/{id}?status=
	public boolean updateDeliveryStatus(@PathVariable Long id, @RequestParam DeliveryStatusEnum status) {
	...
	public Delivery createDelivery(@RequestBody CreateDeliveryDTO createDeliveryDTO) {
		...
		Customer customerFound = customers.stream().filter(customer -> customer.getId().equals(id)).findFirst()
				.orElse(null);
	...
	public ResponseEntity<?> getDeliveryById(@PathVariable Long deliveryId) {
		Delivery deliveryFound = deliveryServiceInterface.getAllDeliveries().stream()
				.filter(delivery -> delivery.getId().equals(deliveryId)).findFirst().orElse(null);
