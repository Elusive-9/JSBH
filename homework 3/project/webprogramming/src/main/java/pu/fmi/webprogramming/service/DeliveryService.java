...
public class DeliveryService implements DeliveryServiceInterface {
	...
	public DeliveryService(DeliveryRepository deliveryRepository, CourierRepository courierRepository,
			WarehouseRepository warehouseRepository, DeliveryEstimator deliveryEstimator) {
	...
	public Delivery assignCourier(Long id, Long courierId) {
		// TODO: Довършване на имплементацията за промяна на доставчика за дадена
		// поръчка
		// (използвай добавената логика в новото REST API - PUT
		// '/api/deliveries/{id}/courier')
		...
		Courier courierFound = courierRepository.getAllCouriers().stream()
				.filter(courier -> courier.getId().equals(courierId)).findFirst().orElse(null);

		if (courierFound == null) {
			throw new DeliveryCustomException("Courier not found.");
		}

		if (!courierFound.isAvailable()) {
			throw new DeliveryCustomException("Courier is not available.");
		}

		delivery.setCourier(courierFound);
		delivery.setDeliveryStatus(DeliveryStatusEnum.ASSIGNED);
		courierFound.setAvailable(false);
	...
	private boolean isStatusValid(DeliveryStatusEnum currentStatus, DeliveryStatusEnum newStatus) {
		...
		if ((CREATED.equals(currentStatus) || ASSIGNED.equals(currentStatus)) && CANCELED.equals(newStatus)) {
			return true;
		}
