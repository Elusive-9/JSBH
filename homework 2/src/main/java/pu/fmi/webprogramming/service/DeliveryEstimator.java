...
public class DeliveryEstimator {
	...
	// ВАЖНО:
	// * Всички предоставени Unit тестове (DeliveryServiceTest) трябва да минават
	// успешно
	...
	public LocalDateTime estimateArrivalTime(Delivery delivery) {
		...
		LocalDateTime deliveryCreationDate = delivery.getCreatedAt();

		long estimatedDays = 1;

		if (!customerCity.equals(warehouseCity)) {
			estimatedDays += 2;
		}

		if (delivery.getDeliveryStatus() == DeliveryStatusEnum.CREATED) {
			estimatedDays += 2;
		}

		return deliveryCreationDate.plusDays(estimatedDays);
	}
}
